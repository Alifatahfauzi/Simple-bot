...
const PAIH_PRODUK = "./settings/dbku/produk.json";
const loadProduk = () => {
    try {
        let data = fs.readFileSync(PAIH_PRODUK, "utf8");
        return JSON.parse(data);
    } catch (e) {
        if (e.code === 'ENOENT') {
            fs.writeFileSync(PAIH_PRODUK, JSON.stringify([]));
            return [];
        }
...
const saveProduk = (produkList) => {
    fs.writeFileSync(PAIH_PRODUK, JSON.stringify(produkList, null, 2));
};
